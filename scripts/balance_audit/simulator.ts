...
import { CARD_DB, COMMON_WEAPONS, COMMON_ARMORS } from "../../src/cards.ts";
import { generateFloorMap as _gfm, getReachableNodes, getNode } from "../../src/map.ts";
import { GAMBLER_OPTIONS, SHRINE_OPTIONS, MERCHANT_PRICES, MERCHANT_SELL_PRICES } from "../../src/events.ts";
import {
...
import { getSuitAffinity, suitTier, getActiveSpecialty } from "../../src/battle.ts";
...
// 全局 lock 通过 SIM_LOCK_SUIT 环境变量传入（balance audit 跑特定流派时用）
const LOCKED_SUIT = (process.env.SIM_LOCK_SUIT as Suit | undefined) || null;
function pickIntendedBuild(): Suit {
  if (LOCKED_SUIT && SUITS.includes(LOCKED_SUIT)) return LOCKED_SUIT;
...
function runOnce(runId: number, opts: SimOptions): RunResult {
  ...
  // 基础装备：COMMON_WEAPONS / COMMON_ARMORS 已经在 STARTING_DECK_IDS 里
  void COMMON_WEAPONS; void COMMON_ARMORS;
    ...
    try {
      switch (state.phase) {
        ...
        case "battle_victory":
          ...
          break;
        case "elite_drop_choice":
          // v0.8.0 新增：精英 SR 掉落 modal
          // AI 默认接受（多数情况精英掉的牌对 build 有用）— 后续可加流派契合度判断
          Game.acceptEliteDrop(state);
          break;
        case "reward_card":
